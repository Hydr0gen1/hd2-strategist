...
| `wiki.ts` | Stage 4 LORE source, pure half (`get_wiki_page`): URL/cache-key builders (intro `prop=extracts` vs full `prop=revisions`; plus the `:wikitext` infobox key), canonical-title resolution, response shaping, mandatory attribution, AND the pure infobox parser (`parseInfobox` / `cleanInfoboxValue` / `extractWikitextContent`) — brace-depth block boundary, top-level field split respecting nested `{{…}}`/`[[…]]`, value cleaning (Damage/Armor/Currency/`{{*}}`/`<br>`/links/`<small>`/italics; unmatched braces stripped, never thrown) | **Pure. Zero I/O. SEPARATE source** — never imports from or feeds into the live war-state pipeline; no live war number in any output. The infobox is community lore like the rest of the payload. |
| `wikiClient.ts` | Stage 4 LORE source, I/O half: helldivers.wiki.gg fetch (fixed descriptive User-Agent) + KV cache in the `wiki:` namespace keyed on the canonical title (24h intro / 1h full TTL). On any fetch failure it throws a typed `WikiError` — no stale fallback, no partial object. The DEFAULT (intro) found path additionally derives `infobox`: a SEPARATE cache-first wikitext load (`wiki:page:{title}:wikitext`, 1h TTL, canonical + alias keys) parsed via `parseInfobox` and attached at the single exit — NEVER baked into the intro cache entry, so it refreshes on the shorter TTL. UNLIKE the primary fetch, every wikitext failure (network/HTTP/non-JSON/KV) is SWALLOWED → empty infobox, never a failed call; `full: true` does NOT load wikitext for infobox (unchanged) | Deliberately separate from `client.ts`. Injectable fetch for tests. Never touches `raw:`/`samples:` keys. |
...
| `archive.ts` | Stage 12 D1 history archive I/O: the best-effort batched per-tick write (`archiveSampleTick` — the item-7 `quarantined` rows and the item-10 `mo_outcomes` rows go in a SECOND, separately-isolated batch, so a missing optional table — migrations 0003/0004 unapplied — can never atomically reject the core planet/global/MO append; the codex-review batch-isolation fix) and the read queries: the long-range trio (`readPlanetArchive` / `readGlobalArchive` / `readMoArchive`, all accepting an optional `untilMs` upper edge — item 2), the item-6 window-edge readers (`readPlanetEdgeRows` / `readMoEdgeRows` / `readGlobalEdgeRow` — single-aggregate GROUP BY, the documented SQLite bare-column semantics — plus `readArchiveCoverage`), the item-10 `readMoOutcomes`, and the item-8 health reads (`readArchiveCounts` / `readGlobalSampleTimestamps` / `readQuarantineSummary`, which degrade to null when a table's migration is unapplied) | **The D1 analog of client.ts (I/O).** APPEND-ONLY archive that lives ALONGSIDE the KV ring buffer, never replacing it — D1 is read ONLY by the archive tools, NEVER by live logic. Write is one CORE `db.batch` per tick (never a per-row loop; plus at most one optional-table batch on the rare ticks producing quarantine/outcome rows), gated by the SAME 60s interval as the KV write (no duplicate rows), wrapped in its own try/catch that swallows (a D1 outage degrades to "tick not archived", never an error). Parameterized SQL ONLY — every value via `.bind()`. The pure row→point delta builders live in `enrichment.ts`. |
...
| `integrity.ts` | Tier 3 pure data-integrity helpers: the item-7 archive plausibility screen (`screenGlobalRow` — the known 18145/0.07364573 sentinel PAIR + a 6σ delta-outlier band over the recent KV series, abstaining below 8 deltas or on zero spread) and the item-8 gap/cadence arithmetic (`buildGapList` / `cadenceStats`) | **Pure. Zero I/O.** The screen FLAGS, never corrects: findings carry both sides of the comparison; the diversion to `quarantined_samples` happens in client.ts/archive.ts. It runs at the D1 archive write ONLY — the KV ring buffer, the response path, and the `allFresh` gate are untouched. |
| `mcp.ts` | JSON-RPC 2.0 protocol + the item-1 resource transport (`resources/read` serves a minted export URI by re-running the same keyset read; `export_archive` results carry a `resource_link` content item) | No domain logic. Domain errors become `isError` tool results, never raw exceptions. |
| `types.ts` | Raw upstream + normalized types | Types only. |
| `index.ts` | Entry/routing + cron entry | POST `/` or `/mcp` (MCP); `GET /export/archive` streams the bulk CSV export (`export.ts`); the `scheduled` handler (Cron Trigger, always UTC) delegates to `runScheduledSample` in tools.ts — the request path's own loader and store write, never a fork; failures are swallowed (no user watches a cron tick). |
...
API); `get_wiki_page` answers WHAT IT MEANS (community lore from
helldivers.wiki.gg, CC BY-NC-SA 4.0, attribution mandatory on every found
payload). It takes any page `title` (weapons, warbonds, stratagems, enemies,
...
instead of refetching. A not-found page is never cached. The DEFAULT (intro)
found response also carries a structured `infobox` (`{ type, fields }`) parsed
from the page wikitext: `type` is the detected family (`weapon` | `warbond` |
`armor` | `stratagem` | null) and `fields` are the lightly-cleaned `| key =
value` pairs (only the FIRST infobox, `image`/`caption-image` skipped, empty
values dropped). The wikitext is loaded from its OWN `wiki:page:{title}:wikitext`
key (1h TTL, canonical + alias) — fetched once on a default request, never on a
`full` request, and never baked into the intro cache. A wikitext failure
degrades to `{ type: null, fields: {} }`, never an error; `infobox` is absent
under `full: true`. The pipelines never touch: live tools must not call
