...
- Stage 4 — `get_wiki_page` (originally `get_planet_wiki`): community lore from helldivers.wiki.gg (MediaWiki root `/api.php`; `prop=extracts` intro by default, `prop=revisions` raw wikitext under `full: true`), a physically separate source with mandatory attribution (CC BY-NC-SA 4.0 per the wiki's own rightsinfo) and a `wiki:page:{canonical}:{intro|full}` KV cache (24h intro / 1h full). Takes any page `title` (weapons, warbonds, stratagems, enemies, biomes, planets, …). Live tools say *what is happening*; the wiki says *what it means*; the two are joined only in the conversation layer.
