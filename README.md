...
A headless Galactic War **MCP server** running as a single Cloudflare Worker. It sits between an MCP client (e.g. Claude) and the Helldivers 2 community API (`api.helldivers2.dev`) as a **correctness layer**: it fetches raw war data, strips known deceptive/cosmetic fields, and exposes clean, strategy-ready data through twenty-three MCP tools.
...
| `get_mo_pace` | **Major Order pace, no verdict:** per objective, the OBSERVED progress rate (latest + mean per-interval deltas over the retained progress samples) beside the REQUIRED rate (`remaining ÷ time_left_hours` — the pace that would exactly reach the target at expiry), plus `remaining`, `time_left_hours`, progress/target. The reader compares the two numbers; there is no on-track/behind field by design. State-at-expiry kinds (`hold_planet`) null both rates with a reason. Read-only |
| `get_gambits` | **The gambit board:** every active defense with its attack-origin planet(s) (inverted observed source→target attack pairs), each origin joined with its live liberation state (`raw_hp`/`max_hp`/`liberation_pct_display_only`/signed `hp_per_hour`) and `is_major_order_target`. Facts only — no gambit-viability score. Read-only; under a campaign outage `defenses` is `null` (unknown), never an asserted-empty board |
| `get_war_diff` | **What changed since N hours ago (D1 archive arithmetic):** each planet's / MO objective's / global counter's FIRST vs LAST archived observation inside a two-edged window (`since_hours` default 24, optional `until_hours`) — tracked-faction changes with before/after, campaigns opened/closed (`campaign_id` turnover), per-planet `delta_health`, `net_health_delta_by_faction`, MO progress deltas, global counter deltas. `insufficient_history` when the window predates the archive. Raw before/after pairs and subtractions, no significance ranking or verdict |
| `get_planet` | Deep dive by `index` or `name`, with `hours_to_resolution` projection, waypoint neighbor context (`neighbors` / `neighbor_summary` / `frontline` adjacency fact), reverse adjacency (`inbound_neighbors` / `adjacency_summary` with the `borders_super_earth` fact), `per_player_rates` (net & gross per-1k-players, consuming the one signed rate), per-region/city sub-objectives (`regions` / `regions_available` / `has_city_region`), the defense `gambit_origin(s)` (the planet attacking this defense — raw state + MO membership, never a viability verdict), `isolation_risk` (the active campaigns whose ONLY adjacent Human-owned planet is this one — the set that loses its last observed Super Earth warp link if this planet flips owner; a one-hop adjacency fact, `null` when campaign state is unknown), and a `cross_check` block verifying the normalized fields against the raw ArrowHead status (both values surfaced on any disagreement, never resolved; degrades to a reasoned null when `/raw` is unavailable). Serves the most recent cached bulk snapshot (`stale: true`) when a live fetch cannot complete. **Persistence requires a complete live fetch:** a stale/snapshot/outage observation is served but never recorded (no history/archive write), and during a campaign-fetch outage campaign state is `campaign_state_known: false` with `has_active_campaign: null` — never asserted false |
| `get_supply_graph` | Supply-line graph over **observed waypoint edges**. No args → the active-campaign subgraph (every active-campaign planet plus its one-hop inbound+outbound neighbors); `full: true` → the whole galaxy; `root` (index/name) + `depth` (default 1, cap 3) walks outward; `active_only` narrows nodes to active campaigns. Returns `nodes` (with `borders_super_earth`, `campaign_state_known`, and `sole_link_dependents` — the active-campaign planets whose only Human-owned neighbor is this node) and directed `edges` (`observed: true` — implied reverse edges are never synthesized). **Read-only** (records nothing). Staleness names its source: a structured `provenance` block separates planet-list provenance (`planet_provenance: live_fresh\|live_expired_cache\|snapshot_fallback`, governs topology) from campaign-overlay provenance (`campaigns: ok\|stale\|unavailable`), and `active_campaign_overlay` (`complete\|degraded\|unavailable`) states the overlay's trust. Under a campaign outage the topology stays complete while nodes are `campaign_state_known: false` — an empty active subgraph reads as *unknown*, never "no active campaigns" |
...
| `get_wiki_page` | **Lore source (separate from live war state):** community wiki entry from helldivers.wiki.gg for any topic by `title` (weapons, warbonds, stratagems, enemies/subfactions like "Jet Brigade", boosters, passives, missions, biomes, planets). Returns the plain-text intro extract by default plus a structured `infobox` (`{ type, fields }`) parsed from the page wikitext (key-value stats with inner templates resolved; `type` ∈ weapon/warbond/armor/stratagem or null); `full: true` returns the raw page wikitext (no `infobox`). Carries the canonical URL and mandatory attribution (CC BY-NC-SA 4.0). Never authoritative for current war state |
...
| `get_planet_archive` | **Long-range (D1 archive):** the unbounded counterpart to `get_planet_history` — a planet's observed health series read from the durable D1 store (`index` or `name`, optional `since_hours` default 168 / `until_hours` closing the window's upper edge for paging older bands / `limit` cap 1000), with per-point `delta_health`/`delta_hours` and the stored signed `hp_per_hour`. Observed points and deltas only, never a forecast |
| `get_global_archive` | **Long-range (D1 archive):** the unbounded counterpart to `get_global_history` — global war statistics over days/weeks (player count, `impact_multiplier`, `active_campaign_count`, missions, deaths, kills) with raw observed deltas (`since_hours`/`until_hours` bound a two-edged window for paging older bands). The view that answers impact-multiplier-vs-population and the daily population cycle; no correlation or model, ever |
| `get_major_order_archive` | **Long-range (D1 archive):** the unbounded counterpart to `get_major_order_history` — Major Order objective progress across a whole order, one series per objective with `delta_progress`/`delta_hours` and `progress_pct`. Optional `major_order_id`/`objective_index`/`until_hours`. Also serves `outcomes` — the completed-order log: each past MO objective's final observed progress/target and `target_reached` (the plain comparison at the last observation). Observed samples and deltas only, never a forecast, required pace, or verdict (`objective_kind` is `null` here — the raw task type is not archived; use `get_major_order_history` for the label) |
| `get_health` | **The server's self-report:** archive row counts per table, archive coverage, the recent gap list (spacings > 15 min between archived global samples), cadence adherence against the 10-minute cron, and quarantine tallies (rows the plausibility screen diverted from the live archive — each with its reason, both sides of the comparison, and the excluded row verbatim). Deterministic counts and spacing facts; a gap means *nothing was recorded there* (outage or a degraded tick that correctly recorded nothing — the archive cannot attribute which) |
| `export_archive` | **Bulk CSV export (bypasses the 1000-row cap):** pull the *whole* D1 archive (or an arbitrary window) for off-context analysis. Pick `table` (`global`/`planet`/`mo`); optional `since`/`until` (ISO-8601) or `since_hours`/`until_hours` bound an arbitrary window (both edges); `planet_index` filters the planet table; `bucket` (`raw`/`hourly`/`daily`) server-side rolls up long ranges (mean of rates/multiplier, last value of counts). Returns **metadata plus an MCP `resource_link`** for the same frozen snapshot — read the link (`resources/read`) to receive the full CSV through the MCP connector (works when direct HTTP egress to the Worker is blocked), or stream the returned `url` over HTTP (`GET /export/archive`); the rows are never inlined in the tool result. A faithful verbatim dump of stored rows: no derived/trend columns, analysis stays in the conversation layer |
...
# 3. Apply the schema (every file in migrations/) — BOTH local and remote.
...
src/mcp.ts         JSON-RPC 2.0: initialize, tools/list, tools/call, resources/read (the export resource_link)
src/client.ts      Upstream fetch + KV cache (raw responses) + rate sampling; triggers the D1 archive write
src/archive.ts     D1 history archive I/O — best-effort batched write (incl. quarantine + MO outcomes) + the long-range/edge/health read queries (Stage 12+)
...
src/enrichment.ts  Pure fact pass-throughs (stats, timing, dispatches, history deltas, event decode, archive points, MO pace, isolation facts, war diff)
src/integrity.ts   Pure data-integrity helpers — the archive plausibility screen (sentinel + Nσ outlier) and gap/cadence arithmetic
...
migrations/        D1 schema migrations (0001_init, 0002 planet_samples time index, 0003 quarantine, 0004 mo_outcomes) applied via `wrangler d1 migrations apply`
```
